...
from nexus.database.enterprise_features import (
...
class APIConnector(DataConnector):
    ...
    def connect(self):
        ...
        self.logger.info("API session created")
    ...
    def extract(self) -> pd.DataFrame:
        ...
        try:
            if not isinstance(self.config, DataSource):
                raise ValueError("Config must be DataSource for extraction")
    ...
    def load(self, df: pd.DataFrame):
        ...
        try:
            ...
            self.logger.info(f"Loaded {len(records)} rows to API")
        except Exception as e:
